    <!-- banner&links -->
<div class="container-fluid full link">
    <div  class="carou">
        <h1>Kết quả tìm kiếm</h1>
	    <ul class="breadcrumb text-center">
	    	<li><a href="./">Trang chủ</a></li>
	    	<li class="active"><a href="javascript:void(0)">Tìm kiếm</a></li>
	    </ul>
	</div>
</div>
	<!-- banner&links -->

    <div class="de px-5 py-3">
        <div class="row justify-content-between">
            <div class="col-6 col-sm-6 col-md-5 col-lg-4 col-xl-4 pl-5">
                <h4  style="display:contents;">KẾT QUẢ TÌM KIẾM:</h4>
            </div>
            <div class="col-5 col-sm-4 col-md-3 col-lg-3 col-xl-2">
                <div class="wrap">
                    <button class="loc px-1 py-1" onclick="loc()"><a data-toggle="collapse" href="#collapse1"><i class="fas fa-filter"></i> Bộ lọc </a></button>

                    <div id="collapse1" class=" divloc panel-collapse collapse">
                        <form class="pl-2" action="" method="get">
                            <div class="py-2">
                                <label for="cars">Lọc theo:</label>
                                <select name="cars" id="cars">
                                    <option value="volvo">Ưu chuộng</option>
                                    <option value="saab">Giá</option>
                                    <option value="opel">Đánh giá cao</option>
                                </select>
                            </div>
                            <hr class="my-2">
                            <div class="py-2">
                                <label for="cars">Sắp xếp:</label>
                                <select name="cars" id="cars">
                                    <option value="volvo">Tăng dần</option>
                                    <option value="saab">Giảm dần</option>
                                </select>
                            </div>
                        </form>
                    </div>
                </div>
            </div>
        </div>
    </div>

	<div class="container-fluid">
        <div class="contentProducts__card">
            <div class="row mx-0 px-3">
                <div class="col-6 col-sm-4 col-md-4 col-lg-3 col-xl-3 pt-5">
                    <div class="contentProducts__card anime ">
                        <div class="contentProducts__image">
                            <div class="contentProducts__label">Mới!</div>
                            <a href="#"><img src="public/img/cake.png" alt="undefined"></a>
                        </div>
                        <div class="contentProducts__text">
                                <h4 class="contentProducts__productTitle text-center">Bánh xe</h4>
                                <div class="contentProducts__priceContainer row">
                                    <div class="col-8 col-sm-8 col-md-8 col-lg-8 col-xl-8">
                                        <a href="#">
                                        <div class="contentProducts__priceFinal">590.000đ</div>
                                        </a>
                                        <sub class="del">690.000đ</sub>
                                    </div>
                                    <div class="col-4 col-sm-4 col-md-4 col-lg-4 col-xl-4">
                                        <div class="contentProducts__priceOriginal pl-2">
                                            <i title="Thêm vào yêu thích" class=" faheart fa fa-heart" aria-hidden="true"></i>
                                        </div>
                                    </div>
                                </div>
                            <div class="contentProducts__info">
                                <div class="row web-kit">
                                    <div class="col-6 px-0">
                                        <div class="contentProducts__link">
                                            <a href="#">
                                                <span title="Thêm vào giỏ hàng" class="org">Thêm vào giỏ
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-circle-plus" width="44" height="44" viewBox="0 0 24 24" stroke-width="1.5" stroke="#698ccd" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                                    <path stroke="none" d="M0 0h24v24H0z"></path>
                                                    <circle cx="12" cy="12" r="9"></circle>
                                                    <line x1="9" y1="12" x2="15" y2="12"></line>
                                                    <line x1="12" y1="9" x2="12" y2="15"></line>
                                                    </svg>
                                                </span>
                                                <i class="icon-plus fas fa-cart-plus"></i>
                                            </a>
                                        </div>
                                    </div>
                                    <div class="col-6">
                                        <div class="contentProducts__view cta cta02" title="Mua">Mua</div>
                                    </div>
                                </div>    
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col-6 col-sm-4 col-md-4 col-lg-3 col-xl-3 pt-5">
                </div>
                <div class="col-6 col-sm-4 col-md-4 col-lg-3 col-xl-3 pt-5">
                </div>
                <div class="col-6 col-sm-4 col-md-4 col-lg-3 col-xl-3 pt-5">
                    <div class="contentProducts__card anime ">
                        <div class="contentProducts__image">
                            <a href="#"><img src="public/img/cake.png" alt="undefined"></a>
                        </div>
                        <div class="contentProducts__text">
                                <h4 class="contentProducts__productTitle text-center">Bánh xe</h4>
                                <div class="contentProducts__priceContainer row">
                                    <div class="col-8 col-sm-8 col-md-8 col-lg-8 col-xl-8">
                                        <a href="#">
                                        <div class="contentProducts__priceFinal">590.000đ</div>
                                        </a>
                                        <sub class="del">690.000đ</sub>
                                    </div>
                                    <div class="col-4 col-sm-4 col-md-4 col-lg-4 col-xl-4">
                                        <div class="contentProducts__priceOriginal pl-2">
                                            <i title="Thêm vào yêu thích" class=" faheart fa fa-heart" aria-hidden="true"></i>
                                        </div>
                                    </div>
                                </div>
                            <div class="contentProducts__info">
                                <div class="row web-kit">
                                    <div class="col-6 px-0">
                                        <div class="contentProducts__link">
                                            <a href="#">
                                                <span title="Thêm vào giỏ hàng" class="org">Thêm vào giỏ
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-circle-plus" width="44" height="44" viewBox="0 0 24 24" stroke-width="1.5" stroke="#698ccd" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                                    <path stroke="none" d="M0 0h24v24H0z"></path>
                                                    <circle cx="12" cy="12" r="9"></circle>
                                                    <line x1="9" y1="12" x2="15" y2="12"></line>
                                                    <line x1="12" y1="9" x2="12" y2="15"></line>
                                                    </svg>
                                                </span>
                                                <i class="icon-plus fas fa-cart-plus"></i>
                                            </a>
                                        </div>
                                    </div>
                                    <div class="col-6">
                                        <div class="contentProducts__view cta cta02" title="Mua">Mua</div>
                                    </div>
                                </div>    
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="row mx-0 px-3">
                <div class="col-6 col-sm-4 col-md-4 col-lg-3 col-xl-3 pt-5">
                    <div class="contentProducts__card anime ">
                        <div class="contentProducts__image">
                            <div class="contentProducts__label">Mới!</div>
                            <a href="#"><img src="public/img/cake.png" alt="undefined"></a>
                        </div>
                        <div class="contentProducts__text">
                                <h4 class="contentProducts__productTitle text-center">Bánh xe</h4>
                                <div class="contentProducts__priceContainer row">
                                    <div class="col-8 col-sm-8 col-md-8 col-lg-8 col-xl-8">
                                        <a href="#">
                                        <div class="contentProducts__priceFinal">590.000đ</div>
                                        </a>
                                        <sub class="del">690.000đ</sub>
                                    </div>
                                    <div class="col-4 col-sm-4 col-md-4 col-lg-4 col-xl-4">
                                        <div class="contentProducts__priceOriginal pl-2">
                                            <i title="Thêm vào yêu thích" class=" faheart fa fa-heart" aria-hidden="true"></i>
                                        </div>
                                    </div>
                                </div>
                            <div class="contentProducts__info">
                                <div class="row web-kit">
                                    <div class="col-6 px-0">
                                        <div class="contentProducts__link">
                                            <a href="#">
                                                <span title="Thêm vào giỏ hàng" class="org">Thêm vào giỏ
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-circle-plus" width="44" height="44" viewBox="0 0 24 24" stroke-width="1.5" stroke="#698ccd" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                                    <path stroke="none" d="M0 0h24v24H0z"></path>
                                                    <circle cx="12" cy="12" r="9"></circle>
                                                    <line x1="9" y1="12" x2="15" y2="12"></line>
                                                    <line x1="12" y1="9" x2="12" y2="15"></line>
                                                    </svg>
                                                </span>
                                                <i class="icon-plus fas fa-cart-plus"></i>
                                            </a>
                                        </div>
                                    </div>
                                    <div class="col-6">
                                        <div class="contentProducts__view cta cta02" title="Mua">Mua</div>
                                    </div>
                                </div>    
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>    
    </div>
    <div class="container-fluid">
        <span class="text-center pt-3"><h3><-- Muốn làm phân trang chổ ni --></h3></span>
    </div>


